...
export default function App() {
  ...
  function getAnswerStatus(num) {
    console.log("Calculating answer status...");
    console.log("num:", num);
    console.log("mathProblem.answer:", mathProblem.answer);

    const tolerance = 0.01; // You can adjust the tolerance based on your needs

    if (Math.abs(num - mathProblem.answer) < tolerance) {
      console.log("Answer: Doğru!");
      return "Doğru!";
    } else if (num < mathProblem.answer) {
      console.log("Answer: Düşük");
      return "Düşük";
    } else {
      console.log("Answer: Yüksek");
      return "Yüksek";
    }
  }
  ...
  function handleSubmit(e) {
    ...
    // Kullanıcının cevabını matematik sorusuyla karşılaştır ve durumu belirle
    setMathProblem((prevData) => ({
      ...prevData,
      submittedResponse: userAnswer
    }));
    setAnswerStatus(() => getAnswerStatus(userAnswer));
    setShowAnswerStatus(true); // Ekranın bu kısmını göster
    setRecentStatusChange(true);
    // Eğer kullanıcı doğru cevap verdiyse, doğru cevap animasyonunu göster ve yeni soru al
